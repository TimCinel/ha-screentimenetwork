...
async def test_api_get_data_success():
    ...
    mock_resp.json = AsyncMock(return_value={"data": {"totalScreenTime": 150.5}})
    ...
    client = STNApiClient(
        handle="testuser", api_key="test_api_key", session=mock_session
    )
...
async def test_api_auth_error():
    ...
    client = STNApiClient(
        handle="testuser", api_key="invalid_key", session=mock_session
    )
